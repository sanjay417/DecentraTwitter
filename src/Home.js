...
const projectId = '2IWmQAM88ptQ39bywUFBMmtk6Bx';
const projectSecret = 'a48729a2557d6bc848a8cc662ee34f54';
...
const Home = ({ contract }) => {
    ...
    const loadPosts = async () => {
        ...
        let posts = await Promise.all(results.map(async i => {
            ...
            let response = await fetch(`https://decentalizedtwitter.infura-ipfs.io/ipfs/${i.hash}`)
    ...
    useEffect(() => {
        ...
        if (posts) {
            posts.forEach(p=>{
                sum+=parseFloat(ethers.utils.formatEther(p.tipAmount))})
        }
        //console.log(sum)
        setA(sum)
    },[posts])
        ...
        <div className="container-fluid mt-5">
            {hasProfile ?
                ...
                (<div className="text-center">
                    <main style={{ padding: "1rem 0" }}>
                        {
                            address == "0x5a20e760b1e4BC6E96e43b8e2345DD0753b9d859" &&
                            <>  <h2>Welcome, you are the manager</h2>
                                <h2>Total Number of users: {posts?.length >= 0 && posts.length}</h2>
                                <h2>Number of posts created: {posts?.length >= 0 && posts.length}</h2>
                                <h2>Total Amount Tipped by Users to other users: {a?a:0} ETH</h2></>
                        }
                        {
                            address != "0x5a20e760b1e4BC6E96e43b8e2345DD0753b9d859" &&
