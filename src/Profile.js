...
const App = ({ contract }) => {
    ...
    const uploadToIPFS = async (event) => {
        ...
        if (typeof file !== 'undefined') {
            try {
                ...
                setAvatar(`https://decentalizedtwitter.infura-ipfs.io/ipfs/${result.path}`)
            } catch (error) {
    ...
    const mintProfile = async (event) => {
        ...
        try {
            ...
            setLoading(true)
            await (await contract.mint(`https://decentalizedtwitter.infura-ipfs.io/ipfs/${result.path}`)).wait()
            loadMyNFTs()
        } catch (error) {
